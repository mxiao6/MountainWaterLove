...
float4 TexCoords_VC( VertexInput_VC v ) {
    ...
    texcoord.zw =
        TRANSFORM_TEX( ( ( _UVSec == 0 ) ? v.uv0 : v.uv1 ), _DetailAlbedoMap );
    return texcoord;
}

inline half4 VertexGIForward_VC( VertexInput_VC v, float3 posWorld,
                                 half3 normalWorld ) {
    half4 ambientOrLightmapUV = 0;
// Static lightmaps
#ifndef LIGHTMAP_OFF
    ambientOrLightmapUV.xy =
        v.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
...
// Sample light probe for Dynamic objects only (no static or dynamic lightmaps)
    ...
    ambientOrLightmapUV.zw =
        v.uv2.xy * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
#endif
...
struct VertexOutputForwardBase_VC {
    ...
    half4  tangentToWorldAndParallax[3]
        : TEXCOORD2;  // [3x3:tangentToWorld | 1x3:viewDirForParallax]
...
#ifdef _TANGENT_TO_WORLD
    float4 tangentWorld =
        float4( UnityObjectToWorldDir( v.tangent.xyz ), v.tangent.w );
...
half4 fragForwardBase_VC( VertexOutputForwardBase_VC i ) : SV_Target {
    ...
    UnityGI gi =
        FragmentGI( s, occlusion, i.ambientOrLightmapUV, atten, mainLight );

    half4 c = UNITY_BRDF_PBS( s.diffColor, s.specColor, s.oneMinusReflectivity,
                              s.oneMinusRoughness, s.normalWorld, -s.eyeVec,
    ...
    // float noiseValue     = tex2D( _NoiseTex, i.ambientOrLightmapUV ).r;
    ...
    if ( _Time.y < ( _StartTime + _Duration ) ) {
        ...
        DissolveFactor =
            saturate( ( _Time.y - _StartTime - _Duration ) / _DissolveSpeed );
        if ( noiseValue <= DissolveFactor ) {
            discard;
        }
    }
...
struct VertexOutputForwardAdd_VC {
    ...
    float4 tex : TEXCOORD0;
    half3  eyeVec : TEXCOORD1;
    half4  tangentToWorldAndLightDir[3]
        : TEXCOORD2;  // [3x3:tangentToWorld | 1x3:lightDir]
    LIGHTING_COORDS( 5, 6 )
    UNITY_FOG_COORDS( 7 )
...
VertexOutputForwardAdd_VC vertForwardAdd_VC( VertexInput_VC v ) {
    ...
    float3 lightDir =
        _WorldSpaceLightPos0.xyz - posWorld.xyz * _WorldSpaceLightPos0.w;
...
#ifdef _VERTEXCOLOR
    o.color = lerp( fixed4( 1, 1, 1, 1 ), fixed4( 1, 1, 1, 1 ) * v.color,
                    _IntensityVC );
#endif
...
half4 fragForwardAdd_VC( VertexOutputForwardAdd_VC i ) : SV_Target {
    ...
    UnityLight light = AdditiveLight( s.normalWorld, IN_LIGHTDIR_FWDADD( i ),
                                      LIGHT_ATTENUATION( i ) );
    ...
    UNITY_APPLY_FOG_COLOR(
        i.fogCoord, c.rgb,
        half4( 0, 0, 0, 0 ) );  // fog towards black in additive pass
    return OutputForward( c, s.alpha );
}
...
#ifndef LIGHTMAP_OFF
    o.ambientOrLightmapUV.xy =
        v.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
#elif UNITY_SHOULD_SAMPLE_SH
    o.ambientOrLightmapUV.rgb =
        ShadeSHPerVertex( normalWorld, o.ambientOrLightmapUV.rgb );
#endif
#ifdef DYNAMICLIGHTMAP_ON
    o.ambientOrLightmapUV.zw =
        v.uv2.xy * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
#endif
...
void fragDeferred_VC(
    ...
    : SV_Target0,  // RT0: diffuse color (rgb), occlusion (a)
      out half4 outSpecSmoothness
    : SV_Target1,  // RT1: spec color (rgb), smoothness (a)
      out half4 outNormal
    : SV_Target2,  // RT2: normal (rgb), --unused, very low precision-- (a)
      out half4 outEmission
    : SV_Target3  // RT3: emission (rgb), --unused-- (a)
    ) {
#if ( SHADER_TARGET < 30 )
    outDiffuse        = 1;
    outSpecSmoothness = 1;
    ...
    UnityGI gi = FragmentGI( s, occlusion, i.ambientOrLightmapUV, atten,
                             dummyLight, sampleReflectionsInDeferred );

    half3 color =
        UNITY_BRDF_PBS( s.diffColor, s.specColor, s.oneMinusReflectivity,
                        s.oneMinusRoughness, s.normalWorld, -s.eyeVec, gi.light,
                        gi.indirect )
            .rgb;
#if _VERTEXCOLOR
...
#endif
    color += UNITY_BRDF_GI( s.diffColor, s.specColor, s.oneMinusReflectivity,
                            s.oneMinusRoughness, s.normalWorld, -s.eyeVec,
                            occlusion, gi );
